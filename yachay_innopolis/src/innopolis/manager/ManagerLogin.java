...
public class ManagerLogin implements Serializable {
	...
	// buscar usuarios por ID
		public Usuario UsuarioByCedula(int id_usr) throws Exception {
			return (Usuario) mDAO.findById(Usuario.class, id_usr);
		}
	...
	public int getContUsr() throws Exception {
		...
		try {
			...
			e.printStackTrace();
			throw new Exception("Revise el parametro contador 'usuario': "
					+ e.getMessage());
		}
		return contUSR;
	}
	...
	public void actualizarContadorUSR(int valor) throws Exception {
		...
		try {
			...
			e.printStackTrace();
			throw new Exception("Error al actualizar el contador 'usuario': "
					+ e.getMessage());
		}
	}
	...
	/*
	 * /editar los usuarios public void editarusuario(Integer id_usr, String
	 * correo, String password, Integer tipo /*, Integer[] listTipo) throws
	 * Exception{ Usuario usr = this.UsuarioByID(id_usr); ;
	 * usr.setCorreo(correo); usr.setPassword(password); usr.setTipo(t);
	 * mDAO.actualizar(usr); }
	 */
	...
	public Inter findByInter(Integer id_act, Integer id_tipo) {
		...
		for (Inter i : listado) {
			if (i.getActividad().getIdActividad().equals(id_act)
					&& i.getTipo().getIdTipo().equals(id_tipo)) {
				resp = i;
				break;
			}
		}
	...
	public Inter findbyidext() {
		...
		Inter u = new Inter();
		for (Inter t : s) {
			if (t.getActividad().getIdActividad() == act.getIdActividad()
					&& t.getTipo().getIdTipo() == tip.getIdTipo()) {
				u = t;
			}
		}
	...
	// insertar tipo
	public void insertarTipo(String tipo, String descripcion, Integer[] listado)
			throws Exception {
		Tipo te = new Tipo();
		te.setTipo(tipo);
	...
	// Modificar el tipo
	public void editarTipo(Integer id_tipo, String tipo, String descripcion)
			throws Exception {
	 ...
	 * @throws Exception
	 */
	public void modificarDatosUSR(Integer id_usr, String cedula, String nombre,
			String apellido, String correo) throws Exception {
		Usuario usr = this.UsuarioByID(id_usr);
		usr.setCedula(cedula);
		usr.setNombre(nombre);
		usr.setApellido(apellido);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Usuario findUserByAliasAndPass(String alias, String pass)
			throws Exception {
		try {
			List<Usuario> listado = (List<Usuario>) mDAO.findByParam(
					Usuario.class, "o.alias", alias, null);
			...
			throw new Exception("Error al intentar ingresar al sistema, "
					+ e.getMessage());
		}
	}
	...
	public boolean esUsrActivo(Usuario u) {
		boolean resp = false;
		if (u.getTipoestadousr().getNombreestado().toLowerCase()
				.equals("activado")) {
			resp = true;
		}
		return resp;
	 ...
	 * 
	 * @param alias
	 *            nombre del usuario especifico para el acceso
	 * @return
	 */
	 ...
	 * @return
	 */
	public List<Actividad> findActividadesXaliasUSR(String alias)
			throws Exception {
			...
			try {
				...
				for (Inter tipousr : todos) {
					if (tipousr.getTipo().getIdTipo().equals(u.getIdUsr())) {
						listado.add(findActividadByID(tipousr.getActividad()
								.getIdActividad()));
					}
				}
			} catch (Exception e) {
				throw new Exception(
						"Error al cargar los tipos de usuario pertenecientes");
			}
		}
		return listado;
	}
	...
	// insertar sugerencia
	public void insertarSugerencia(Integer id_usr, String sugerencia,
	...
	// metodo para qe el administrador pueda recivir mensajes de solicitudes
	// como inscripciones
	...
	public void editarInteres(Integer id_interes, String nombreinteres)
			throws Exception {
