...
public class ServiciosVirtualesBean implements Serializable {
	...
	public String crearNuevoTipoServicio() {
		try {
			if (this.cnombreestipser(nomservicio) == true) {
				Mensaje.crearMensajeWARN("El nombre se encuentra utilizado");
			} else {
				managerservirt.insertarTipoServicio(getIdestadotipser(),
						nomservicio, url);
				tiposervicio = null;
				url = "";
				estadotipser = managerservirt.findEstadoTiposervByID(1); // numero
																			// id
																			// del
																			// estado
																			// q
																			...
																			// sea
				setNomservicio("");
				Mensaje.crearMensajeINFO("Creado correctamente");
			}
		} catch (Exception e) {
			Mensaje.crearMensajeWARN("Error al ingresar nuevo tipo");
			e.printStackTrace();
		}
		return "";
	}
	...
	public String EliminarServicio(Tiposervicio tpser) {
		try {
			...
			Mensaje.crearMensajeWARN("Error no eliminado");
			e.printStackTrace();
		}
		return "";
	}
	...
	public String ActualizarServicio() {
		...
		try {
			...
			setIdservi(0);
			resp = "crudservicio";
			Mensaje.crearMensajeINFO("Servicio actualizado");
		} catch (Exception e) {
			Mensaje.crearMensajeWARN("Error al modificar servicio");
			e.printStackTrace();
		}
		return resp;
	}
